...
class SolidityProcess extends events_1.EventEmitter {
    constructor(mode, args) {
        ...
        this.debugSocketClient.on('data', (buffer) => {
            ...
            this.domErrors = [];
            let document = this.parser.parseFromString(this.buffer, 'application/xml');
        ...
        if (mode === common_1.Mode.launch) {
            let runtimeArgs = ['--evaluation-timeout', '10'];
            ...
            let processCwd = args.cwd || path_1.dirname(args.program);
            let processEnv = {};
            // use process environment
            for (let env of process.env) {
                processEnv[env] = process.env[env];
            }
            // merge supplied environment
            for (let env of args.env) {
...
exports.SolidityProcess = SolidityProcess;
//# sourceMappingURL=debugger-process.js.map