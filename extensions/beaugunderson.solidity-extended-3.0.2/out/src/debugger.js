...
const debugger_process_1 = require("./debugger-process");
...
class CachedBreakpoint {
    ...
    convertForResponse() {
        const result = new vscode_debugadapter_1.Breakpoint(true, this.line, this.column);
        result.id = this.id;
        return result;
    }
}
class SolidityDebugSession extends vscode_debugadapter_1.DebugSession {
    /**
    ...
    launchRequest(response, args) {
        ...
        this.solidityProcess = new debugger_process_1.SolidityProcess(common_1.Mode.launch, args);
        this.solidityProcess.on('debuggerConnect', () => {
        ...
        if (args.showDebuggerOutput) {
            this.solidityProcess.on('debuggerOutput', (data) => {
    ...
    attachRequest(response, args) {
        ...
        this.solidityProcess = new debugger_process_1.SolidityProcess(common_1.Mode.attach, args);
        this.solidityProcess.on('debuggerConnect', () => {
            this.sendEvent(new vscode_debugadapter_1.InitializedEvent());
            this.sendResponse(response);
        }).on('suspended', result => {
    ...
    configurationDoneRequest(response, args) {
        this.solidityProcess.Run('start');
        this.sendResponse(response);
    }
    setExceptionBreakPointsRequest(response, args) {
        if (args.filters.indexOf('all') >= 0) {
            ...
            this.solidityProcess.Enqueue('catch Exception').then(() => 1);
        }
        else {
            this.solidityProcess.Run('catch off');
        }
        this.sendResponse(response);
    }
    setBreakPointsRequest(response, args) {
        ...
        if (breakpointsToRemove.length > 0) {
            ...
            var removePromises = breakpointsToRemove.map(bp => this.solidityProcess.Enqueue('delete ' + bp.id));
        ...
        if (breakpointsToAdd.length > 0) {
            ...
            var addPromises = breakpointsToAdd.map(bp => {
                ...
                return this.solidityProcess.Enqueue(command);
            });
    ...
    threadsRequest(response) {
        this.solidityProcess.Enqueue('thread list').then(results => {
    ...
    */
    stackTraceRequest(response, args) {
        this.solidityProcess.Enqueue('where').then(results => {
            // drop rdbug frames
    ...
    convertDebuggerPathToClient(serverPath) {
        ...
        let relativePath = path.join(this.requestArguments.cwd, serverPath.substring(this.requestArguments.remoteWorkspaceRoot.length));
        return relativePath;
    }
    switchFrame(frameId) {
        ...
        this.solidityProcess.Run('frame ' + frameId);
    }
    ...
    */
    scopesRequest(response, args) {
        ...
        Promise.all([
            this.solidityProcess.Enqueue('var local'),
            this.solidityProcess.Enqueue('var global'),
        ])
            .then(results => {
            const scopes = new Array();
            scopes.push(new vscode_debugadapter_1.Scope('Local', this._variableHandles.create({
                variables: this.createVariableReference(results[0]),
            }), false));
            scopes.push(new vscode_debugadapter_1.Scope('Global', this._variableHandles.create({
                variables: this.createVariableReference(results[1]),
            }), false));
            response.body = {
                scopes: scopes,
            };
    ...
    variablesRequest(response, args) {
        ...
        let varPromise;
        if (varRef.objectId) {
            varPromise = this.solidityProcess.Enqueue('var i ' + varRef.objectId).then(results => this.createVariableReference(results));
        }
        else {
    ...
    continueRequest(response, args) {
        this.sendResponse(response);
        this.solidityProcess.Run('c');
    }
    nextRequest(response, args) {
        this.sendResponse(response);
        this.solidityProcess.Run('next');
    }
    stepInRequest(response) {
        this.sendResponse(response);
        this.solidityProcess.Run('step');
    }
    pauseRequest(response) {
        this.sendResponse(response);
        this.solidityProcess.Run('pause');
    }
    stepOutRequest(response) {
        this.sendResponse(response);
        // Not sure which command we should use, `finish` will execute all frames.
        this.solidityProcess.Run('finish');
    }
    ...
    */
    evaluateRequest(response, args) {
        ...
        this.solidityProcess.Enqueue('eval ' + args.expression).then(result => {
            response.body = {
                result: result.value
                    ? result.value
    ...
    disconnectRequest(response, args) {
        if (this.solidityProcess.state !== common_1.SocketClientState.closed) {
            this.solidityProcess.Run('quit');
        }
        this.sendResponse(response);
    }
}
vscode_debugadapter_1.DebugSession.run(SolidityDebugSession);
