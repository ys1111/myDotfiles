...
const contract_collection_1 = require("./contract-collection");
const compiler_errors_1 = require("./compiler-errors");
const TWO_MINUTES = 2 * 60 * 1000;
let validationLocked = 0;
let lastSolcErrors = [];
let lastSoliumErrors = [];
function uriToBasename(uri) {
    return path.basename(vscode_uri_1.default.parse(uri).fsPath);
}
function now() {
...
function compilationErrors(filePath, documentText) {
    const contracts = new contract_collection_1.ContractCollection();
    try {
        ...
        const output = solc.compileStandardWrapper(JSON.stringify(input), (importPath) => {
        ...
        if (output) {
            ...
            if (parsedOutput.errors) {
                return parsedOutput.errors
                    .map(compiler_errors_1.errorToDiagnostic)
...
function validate(document, validators = true) {
...
connection.onDidChangeConfiguration(params => {
    ...
    soliditySettings = params.settings.solidity;
    validateAll();
});
connection.onInitialize((result) => {
    rootPath = vscode_uri_1.default.parse(result.rootUri).fsPath;
